...
if m ~= size(row_space, 2) + size(left_null_space, 2)
    sprintf('something is off, the row subspace dimensions do not agree with the matrix dimensions')
end

if n ~= size(column_space, 2) + size(right_null_space, 2)
    sprintf('something is off, the column subspace dimensions do not agree with the matrix dimensions')
end
end